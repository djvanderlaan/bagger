...
int main(int argc, char **argv) {
  ...
  xmlNode* node = find_first(root->children, "Pand");

  while(node = find_first(node, "Pand")) {
    std::cout << '"' << get_data(node->children, "identificatie") << '"' << ';';
    std::cout << '"' << get_data(node->children, "bouwjaar") << '"' << ';';
    std::cout << '"' << get_data(node->children, "pandstatus") << '"' << ';';
    ...
    xmlNode* geom = find_first(node->children, "pandGeometrie");
    if (geom) {
